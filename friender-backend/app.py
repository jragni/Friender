...
# TODO: import a library to change headers on the backend
# flask-cors
# --- manually change headers to set access-origin-header to *
...
def signup():
    ...
    if form.validate_on_submit():
        print("Pre validation of submit")
        try:
            user = User.signup(
            ...
            print("PASSED VALIDATION OF FORM")
            db.session.commit()
            ...
            return jsonify(failed=f"failed + {e}")
...
def get_profile():
    ...
    profiles = [
        profile for profile in nearby_profiles if profile not in do_not_include]
...
def match():
    ...
    first_match = Match.query.with_entities(
        Match.second_id).filter(Match.first_id == g.user.id).all()
    second_match = Match.query.with_entities(
        Match.first_id).filter(Match.second_id == g.user.id).all()
    matches_id = []
...
"""
  NEEDS JSON with ID of the the other user
  ...
  "messages": [
    {
      ...
      "sender_id": 1,
      "text": "Hello",
      "timestamp": "Fri, 27 Aug 2021 19:18:55 GMT"
    },
    {
      "id": 2,
      "receiver_id": 1,
      "sender_id": 2,
      "text": "sup",
      "timestamp": "Fri, 27 Aug 2021 19:25:25 GMT"
    },
    {
      "id": 3,
      "receiver_id": 2,
...
{
		"id": "1",
		"message": "got any bomb ass animes"
}
...
def send():
    ...
    if (g.user.id == int(sent_to)):
        return jsonify(error="can not send message to yourself")
...
def upload():
    ...
    print(incoming_request)
    return jsonify(request=incoming_request)
